...
(** Correctness proof for LoongArch generation: main proof. *)
...
Remark opsi32_label:
  ...
  tail_nolabel k (opsi32 op opimm r1 r2 n k).
Proof.
...
Remark opui32_label:
  ...
  (forall r1 r2 r3, nolabel (op r1 r2 r3)) ->
  (forall r1 r2 n, nolabel (opimm r1 r2 n)) ->
  tail_nolabel k (opui32 op opimm r1 r2 n k).
Proof.
...
Remark opui64_label:
  ...
  (forall r1 r2 r3, nolabel (op r1 r2 r3)) ->
  (forall r1 r2 n, nolabel (opimm r1 r2 n)) ->
  tail_nolabel k (opui64 op opimm r1 r2 n k).
Proof.
...
Proof.
  ...
  destruct Archi.ptr64. apply opsi64_label; TailNoLabel. apply opsi32_label; TailNoLabel.
Qed.
...
- destruct (Int.eq n Int.zero).
  destruct c0; simpl; TailNoLabel.
  apply tail_nolabel_trans with (transl_cbranch_int32s c0 x R20 lbl :: k).
  auto with labels. destruct c0; simpl; TailNoLabel.
- destruct (Int.eq n Int.zero).
  destruct c0; simpl; TailNoLabel.
  apply tail_nolabel_trans with (transl_cbranch_int32u c0 x R20 lbl :: k).
  auto with labels. destruct c0; simpl; TailNoLabel.
- destruct c0; simpl; TailNoLabel.
- destruct c0; simpl; TailNoLabel.
- destruct (Int64.eq n Int64.zero).
  destruct c0; simpl; TailNoLabel.
  apply tail_nolabel_trans with (transl_cbranch_int64s c0 x R20 lbl :: k).
  auto with labels. destruct c0; simpl; TailNoLabel.
- destruct (Int64.eq n Int64.zero).
  destruct c0; simpl; TailNoLabel.
  apply tail_nolabel_trans with (transl_cbranch_int64u c0 x R20 lbl :: k).
  auto with labels. destruct c0; simpl; TailNoLabel.
- destruct (transl_cond_float c0 FCC0 x x0) as [insn normal] eqn:F; inv EQ2.
  apply tail_nolabel_cons. eapply transl_cond_float_nolabel; eauto. 
  destruct normal; TailNoLabel.
- destruct (transl_cond_float c0 FCC0 x x0) as [insn normal] eqn:F; inv EQ2.
  apply tail_nolabel_cons. eapply transl_cond_float_nolabel; eauto. 
  destruct normal; TailNoLabel.
- destruct (transl_cond_single c0 FCC0 x x0) as [insn normal] eqn:F; inv EQ2.
...
+ destruct c0; simpl; TailNoLabel.
+ destruct c0; simpl.
* eapply tail_nolabel_trans; [apply opui32_label; intros; exact I | TailNoLabel].
* eapply tail_nolabel_trans; [apply opui32_label; intros; exact I | TailNoLabel].
* apply opsi32_label; intros; exact I.
* destruct (Int.eq n (Int.repr Int.max_signed)). apply loadimm32_label. apply opsi32_label; intros; exact I.
...
+ destruct c0; simpl; TailNoLabel.
+ destruct c0; simpl.
* eapply tail_nolabel_trans; [apply opui64_label; intros; exact I | TailNoLabel].
* eapply tail_nolabel_trans; [apply opui64_label; intros; exact I | TailNoLabel].
* apply opsi64_label; intros; exact I.
* destruct (Int64.eq n (Int64.repr Int64.max_signed)). apply loadimm32_label. apply opsi64_label; intros; exact I.
...
- apply opsi32_label; intros; exact I.
- apply opui32_label; intros; exact I.
- apply opui32_label; intros; exact I.
- apply opui32_label; intros; exact I.
- destruct (Int.eq n Int.zero); TailNoLabel.
- apply opsi64_label; intros; exact I.
- apply opui64_label; intros; exact I.
- apply opui64_label; intros; exact I.
- apply opui64_label; intros; exact I.
- destruct (Int.eq n Int.zero); TailNoLabel.
...
Proof.
  ...
  simpl. destruct (storeind_ptr_label R1 R3 (fn_retaddr_ofs f) x) as [A B]; rewrite B. 
...
- intros. monadInv H0.
  ...
  constructor. apply (storeind_ptr_label R1 R3 (fn_retaddr_ofs f0) x).
...
Inductive match_states: Mach.state -> Asm.state -> Prop :=
  | match_states_intro:
      forall s fb sp c ep ms m m' rs f tf tc
        ...
        (AG: agree ms sp rs)
        (DXP: ep = true -> rs#R19 = parent_sp s),
...
Lemma exec_straight_steps:
  ...
  (forall k c (TR: transl_instr f i ep k = OK c),
   exists rs2,
    ...
    /\ agree ms2 sp rs2
    /\ (it1_is_parent ep i = true -> rs2#R19 = parent_sp s)) ->
  exists st',
...
Remark preg_of_not_R19: forall r, negb (mreg_eq r Machregs.R19) = true -> IR R19 <> preg_of r.
Proof.
  intros. change (IR R19) with (preg_of Machregs.R19). red; intros.
...
(* GPR11 does not contain parent *)
  ...
  instantiate (1 := rs1#R19 <- (rs2#R19)). intros.
  rewrite Pregmap.gso; auto with asmgen.
  congruence.
  intros. unfold Pregmap.set. destruct (PregEq.eq r' R19). congruence. auto with asmgen.
...
- (* Mop *)
  ...
  rewrite R; auto. apply preg_of_not_R19; auto.
...
- (* Mjumptable *)
  ...
  instantiate (2 := rs0#R12 <- Vundef #R20 <- Vundef).
  Simpl. eauto.
  eauto.
  intros [tc' [rs' [A [B C]]]].
...
Proof.
  ...
  generalize (preg_val _ _ _ Machregs.R4 AG). rewrite H2. intros LD; inv LD. auto.
Qed.
